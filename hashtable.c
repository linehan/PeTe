...
#define TOMB  ((uint64_t)(0xFFFFFFFFFFFFFFFF))
#define LEN   1
...
static struct hashentry *hash_ent(struct hashtable *table, uint64_t idx)
{
...
{
        table->entry = calloc(1, sizeof(struct hashentry)*n);
        table->capacity = n;
        table->size = 0;
}
